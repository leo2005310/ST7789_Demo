...
初始位置自动分开放置，碰到动画区域边缘或其他方块时反弹，不会重叠穿过。
方块接触时交换碰撞方向上的速度，沿接触面方向的速度保持不变（所有方块按等质量处理）。
...
| `kBoxCount` | 8 | 方块数量，1～32；还需满足屏幕布局容量，越多则计算和绘制开销越大 |
...
所有刷新区域都来自碰撞处理后的同一幅完整画面，避免局部刷新互相擦除或留下拖影。
信息栏只在读数更新时传输，全屏传输只在启动时执行一次。
这能减少 SPI 传输量与擦除闪烁，但不使用屏幕 TE 同步，不能保证完全消除扫描撕裂。

碰撞采用分轴路径检查，每个小步每轴最多移动 0.5 像素，限制方块停在其他方块之前，
避免高速移动穿透。它是适合 Demo 的顺序更新方案，多物体同时碰撞时结果会受更新顺序影响。
初始网格为每个方块预留至少 2 像素的单元余量；默认 240×240、信息栏 24 像素、
最大方块 36 像素时最多能排下 30 个。超出布局容量时屏幕和串口提示错误，需减少数量或尺寸。

运动、碰撞与矩形计算位于 `include/animation.h`，不依赖 Arduino；可用本机 C++ 编译器验证：
